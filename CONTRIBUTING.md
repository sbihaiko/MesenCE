...
**Product branch is `main`.** `master` is a frozen full-console snapshot. Upstream (`nesdev-org/MesenCE`) is synced by **merge, never rebase**, under ADR-0163: ownership tiers A/B/C+D are computed mechanically (`scripts/upstream_tiers.py`), and upstream's edits to files this fork deleted — the dropped consoles (SNES incl. SGB, PC Engine, WonderSwan, ColecoVision) and their UI — are resolved as deletions, so a sync brings upstream's *changes* and never reintroduces a dropped console. Do not use GitHub's **Sync fork** button (no rerere/auto-resolution, no PR gate); run `scripts/sync-upstream.sh` locally on `main`, or review the PR the scheduled `sync-upstream.yml` workflow opens when upstream moves. Small bugfixes from upstream can still be cherry-picked file-by-file when a full sync is more than the change warrants. Code taken *from* upstream still follows upstream's style (clang-format / dotnet format) so those cherry-picks stay readable.
