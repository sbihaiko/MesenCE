...
void GbEnhancedSynth::MixAudio(int16_t* out, uint32_t sampleCount, uint32_t sampleRate)
{
	...
	//Raw melodic channels in chip order (square1, square2, wave); the
	//classifier assigns lead/harmony/bass and flags sound effects (ADR-0052),
	//then Route() fills the engine slots. Square volumes are the live envelope
	//output (0-15); Enabled covers length-counter expiry and DAC power.
	//Per-channel volume settings (Settings > Game Boy > Audio) apply to the
	//synth voices too, so muting a chip channel also mutes its enhanced
	//voice. The duty cycle travels with the channel as the pulse width (75%
	//sounds identical to 25%, so it maps back to 0.25). Wave is usually the
	//bass line; its 2-bit volume code shifts samples right by (code - 1),
	//i.e. 100% / 50% / 25%, 0 = mute.
	static constexpr double dutyWidth[4] = { 0.125, 0.25, 0.5, 0.25 };
	EnhancedSynthEngine::RawChannel raw[3];
	raw[0].Freq = squareFreq(apu.Square1.Frequency);
	if(apu.Common.ApuEnabled && apu.Square1.Enabled) {
		raw[0].Vol = apu.Square1.Volume / 15.0 * gbCfg.Square1Vol / 100.0;
	}
