...
public partial class EnhancementPackConfig : BaseConfig<EnhancementPackConfig>
{
	...
	//ADR-0138 §38 (F6.4b): sticky flag for the first-run consent dialog that
	//gates the very first automatic download (UI/Logic/CommunityPackConsentState.cs) -
	//once true it is never asked again while AutoInstallCommunityPacks stays
	//on. UI-only, deliberately NOT mirrored into InteropEnhancementPackConfig/
	//ApplyConfig(): Core never reads AutoInstallCommunityPacks either (grep
	//over Core/ finds only its declaration in SettingTypes.h, no call site) -
	//every consent/download decision is made client-side in
	//UI/Services/CommunityPackInstallService.cs before any native call, so
	//the native side has no need to know this flag exists.
