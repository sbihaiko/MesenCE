	...
	public static class CommunityPackCatalogFetcher
	{
			...
			CommunityPackCatalogEntry entry, IReadOnlyList<CommunityPackHostEntry> allowedHosts)
		{
			...
			foreach(CommunityPackDep dep in entry.Deps) {
				if(dep.UserSupplied == true || string.IsNullOrWhiteSpace(dep.Url) || string.IsNullOrWhiteSpace(dep.Sha256)) {
					continue;
				}
		...
		{
			if(string.IsNullOrWhiteSpace(url) || string.IsNullOrWhiteSpace(expectedSha256)) {
				return null;
			}
			...
			if(File.Exists(destPath) &&
				string.Equals(ComputeSha256(File.ReadAllBytes(destPath)), expectedSha256, StringComparison.OrdinalIgnoreCase)) {
				return destPath;
			}

			return await DownloadToVerifiedFileAsync(url, expectedSha256, destPath);
		}

		private static async Task<string?> DownloadToVerifiedFileAsync(string url, string expectedSha256, string destPath)
		{
			try {
				using HttpClient client = new();
