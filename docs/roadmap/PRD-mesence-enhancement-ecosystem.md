...
3b. **Phase 9 F9.22 → F9.20 → F9.21**, in that order. ADR-0179 accepted
   2026-09-12; ADR-0180 stays `proposed` (both off the Contra side-by-side
   spike). F9.22 first because the 4096-frame cap and the stage-1-only entry
...
| 0179 | accepted (2026-09-12) | `poses.json` gains succession (`next[]`/`hold`), `cycles[]`/`sequences[]` found on the track sequence, and `variantOf` for figure + projectile; the editor lays poses out by cycle. Slice F9.20 |
