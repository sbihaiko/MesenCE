...
- **Per-stage recording, the tooling half** (2026-09-12, F9.22): the
  ...
  wheels all come out as cycles from a 60 s stage-1 run. The states for
  Contra's later stages are the human half and stay open.
...
| F9.21 | **Parts** — rigid tile subsets that recur across >= 2 kept poses become `parts[]`, each pose cites its `composition[]` (part + translation) and a `rest` count; `tiles[]` stays authoritative. The editor shows a pose whole and its parts as labelled regions, joined to the `sprNNN` sheet that stores them. Makes ADR-0171's storage-once rule visible: the artist paints the legs once and sees every figure change, as `BillRizer.png` does with its 3 x 6 grid | **proposed 2026-09-12, open** — ADR-0180 §4: the cover algorithm is unmeasured (naive greedy tiles 24/64 Contra poses with <= 3 of 40 parts), the pose-anchored `spriteNearby` the artist wrote 98 times by hand is a separate ADR, and whether `sprNNN` fragments *are* the parts is unverified. Stays proposed until measured on the golden kit |
| F9.22 | **Per-stage recording roadmap** — the retained OAM stream is capped at 4096 frames (`kMaxSheetFrames`, ~11.5 MB), and gameplay almost never repeats a frame: the 120 s Contra golden run kept 4151 of 7213 emulated frames, so the last 51 s produced no poses and no adjacency. One long run is the wrong shape. Per golden game: a set of `headless_record` save states (`state=<file.mss>`, one per stage / distinct area) plus a <= 60 s entry script each — one run per state into its own pack folder, judged as a union (merging retained streams across runs into one pack is a decision not yet taken and not assumed here), so every subject the artist pack covers (Contra: 8 stages, water, base interior, bosses, player 2) is on screen at least once. Deliverable is the state files, the scripts and a `make`-able batch, not code in the emulator | **tooling shipped 2026-09-12, states pending** — `headless_record` gained `save-state=<file.mss>` (written only when the run reaches its frame target) to pair with `state=`, and `scripts/record_stages.sh <rom> <stages-dir> <out-dir> [seconds]` runs one bootstrap record per `<stage>.mss` + `<stage>.txt` into its own pack folder. Proven on Contra stage 1: a state minted from the entry script and a 60 s script that holds R for 240 f at a time yields the player's period-6 run cycle (`002 009 005 002 011 008`, hold 8) and the somersault 4-cycle that the 120 s golden run never showed as cycles. Run on the whole kit the same day from stage-1 states minted by the entry scripts (`scripts/stages/<game>/`, versioned; states under `runs/`, unversioned — a CHR-RAM state carries ROM-derived graphics): Mega Man 3 8 cycles (the run as `002 005 002 006`, period 4 with the middle frame twice, 17 repeats), Zelda 1 12 (Link's two-frame walk in every direction, hold 6, 40–54 repeats), Excitebike 12 (the wheels, 345 repeats), Contra 2. Cycles with period > 6 and a hold near 100 are echoes of the script's own loop, not animations — a reader should weigh `repeats` and `hold` together. Open: the states for Contra's later stages (water, base interior, bosses, player 2) — no cheat flag is exposed, so they come from a human playing in the GUI from the 30-lives state `mint-stage1-30lives.txt` mints (Konami code, an input), saving a slot at the start of each area |
...
3b. **Phase 9 F9.22 → F9.21**, F9.20 shipped 2026-09-12 (the user accepted
   ADR-0179 and asked for it the same turn); ADR-0180 stays `proposed` (both
   off the Contra side-by-side spike). F9.22 first because the 4096-frame cap and the stage-1-only entry
   script decide the denominator every later measurement divides by; F9.20 is
   derivable from the retained stream with no capture change; F9.21 waits
   for its cover algorithm to be measured on a kit F9.22 has widened.
