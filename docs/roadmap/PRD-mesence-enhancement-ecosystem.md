...
| F5.4g **Block C** | 8 loop point in the fingerprint (`LoopPosition` is 0 today), 9 SFX audible during OGG (mute mask), 10 music→music transition/fade | item 8 done 2026-08-29 (ADR-0134 Option A: `tracks[i].loop`, MEP-v1 §5.2, renderer emits from a MIDI loop marker, lint accepts presence/absence, Bloco H round-trip — 121/121); item 9 done 2026-08-29 (ADR-0133: `SetReplacementMuteMask`, classifier-driven SFX pass-through, bool shim removed, fallback 0x0F, reset clears); item 10 in progress (**ADR-0142 proposed** — 40 ms crossfade on BGM switch/stop) |
| F5.4g **Block D** | 11 *Extract audio* opt-in tool driving the sound driver, longer title→Start window for Contra/SMB1; 12 id naming/cleanup; 13 `mep_build.py` with `audio/`, audio lint, seed-MIDI→OGG tutorial | **ADR-0135** (accepted 2026-08-29) and **ADR-0051** (accepted 2026-08-29) |
...
| 0133, 0134, 0135, 0051 | accepted 2026-08-29 (0134 = Option A: `loop` field in `fingerprints.json`) | unblocks Blocks C/D — implement now |
