...
- Source (1): MEP-v1 §3.1 carries `id` as SHOULD (added in v1.4; the spec is at v1.6); `mep_lint.py:650-652` warns on a missing/malformed `id`. Source (4): `MepPackManager::EffectivePackId` (`MepPackManager.cpp:604-613`) and `PackPreferenceResolver.DerivePackId` (`UI/Logic/PackPreferenceResolver.cs:51-54`).
