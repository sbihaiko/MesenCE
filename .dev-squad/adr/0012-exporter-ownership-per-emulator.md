...
## Alternatives
Keep the process-global singleton with a documented single-emulator contract (defers multi-instance correctness, does not solve attribution or lifecycle). Add locking to the global (fixes races but not attribution or reset/unload semantics). Rejected interim: temporarily keep the static facade with a minimum acceptable hardening — document the single-active-console contract in both headers, have teardown call StopRecording(), and either enforce a single-producer contract explicitly or add a lock around Log()/Stop() keying voice state per console tag. Considered as an expedience fallback while the decision was pending; not taken, since the per-Emulator move shipped directly.
