...
3. **Unified enhancement pack format** — textures + audio + synth preset in one hash-keyed archive, every layer individually toggleable. 🚧 **In progress** — the MEP host core is in: `pack.json` parsing/validation (strict JSON reader, zip-slip rejection), No-Intro SHA-1 matching computed from the ROM file, `EnhancementPacks/` discovery (directories + zips, extracted to a cache) and deterministic precedence (ADR-0038…0042). Sections are delegated to the existing loaders (NES/GB/SMS textures via hires.txt, NES OGG audio, ESP synth preset layered below the user's file), with the loose `HdPacks/` pack taking precedence — validated headless (GB replacement renders 1:1). Next: per-section toggles + UI.
