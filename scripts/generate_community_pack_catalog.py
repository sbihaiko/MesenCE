...
TABLE_HEADER = "| Link | Jogo | Console | Autor | Categoria | Data |"
TABLE_SEP = "|---|---|---|---|---|---|"
...
def build_row(item):
    ...
    if issue_number is None:
        return {"jogo": "(sem issue)", "console": "?", "autor": "?",
    ...
    return {
        "jogo": details.get("title") or "(sem título)",
...
def render_table(rows):
    ...
    if not rows:
        lines.append("| _nenhum pack aceito ainda_ | | | | | |")
    for row in rows:
...
def render_popular_section(rows):
    lines = ["## Mais populares", "",
             "_Ordenado por reações 👍 na issue de submissão. Isso é um proxy de popularidade,_",
             "_não uma métrica real de uso — nenhuma telemetria de uso é coletada por este projeto._", ""]
    ranked = sorted(rows, key=lambda r: r["thumbs_up"], reverse=True)
    if not ranked or ranked[0]["thumbs_up"] == 0:
        lines.append("_nenhum pack com reações ainda._")
        return "\n".join(lines)
    for row in ranked:
        if row["thumbs_up"] <= 0:
            continue
...
INTRO = ("Catálogo gerado automaticamente a partir do board \"MesenCE Community\n"
         "Packs\" (Project 3). Não edite este arquivo manualmente — ele é\n"
         "sobrescrito por `.github/workflows/community-pack-catalog.yml`.")
...
def main():
    ...
    print(f"docs/community-packs.md regenerado com {len(rows)} pack(s) aceito(s).")
    return 0
