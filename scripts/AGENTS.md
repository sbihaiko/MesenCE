...
- `validate-specs.py`, `mep_lint.py`, `mep_recipe.py`, `mep_recipe_assemble.py`,
  ...
  `validate-specs.py` follows the same split convention as
  `mep_recipe.py`/`mep_recipe_common.py` below: `spec_validate_common.py`
  is the dependency-free leaf (`SPECS`, the semver/hash regexes,
  `check()`/`_failures`, `validate_rom_id()`) and `validate_specs_mei.py`
  holds the MEI v1.1 catalog validators (`validate_mei`,
  `validate_mei_catalog`, `MEI_KINDS`); both were split out purely to keep
  `validate-specs.py` under the project's line-count threshold, not
  because of a cycle. Neither leaf imports `validate-specs.py` back (it is
  a script entry point, not an importable module — its filename is
  hyphenated).
