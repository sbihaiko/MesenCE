...
def validate_mei(path):
    ...
    for i, p in enumerate(d.get("packs", [])):
        ...
        # MEI v1.4 §2.6: additive `errata` — exact known-missing targets declared by the index maintainer's validation (ADR-0152), never wildcards.
        for msg in mei_errata_field_errors(p):
            check(False, f"{where}: {msg}")
...
def main():
    ...
    if _failures:
        ...
        sys.exit(1)
    print("validate-specs: all golden files conform (ESP, MEP, MEI v1.4, MEP-recipe, hires-gbsms draft, path-cases format); "
